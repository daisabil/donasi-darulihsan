...
function ActionMenu({ onEdit, onDelete }: { onEdit: () => void; onDelete: () => void }) {
  ...
  const dropdown = open ? (
    <>
      ...
      <div className="action-menu-dropdown" style={menuStyle}>
...
function InputTetapSection({ passcode, fixedDonors, onSuccess, onRefreshDonors }: {
  ...
  const [formBaru, setFormBaru] = useState({ date: new Date().toISOString().split('T')[0], name: '' });
  const [loading, setLoading] = useState(false);
  const [msg, setMsg] = useState<{ type: 'ok' | 'err'; text: string } | null>(null);
  ...
  const handleRegistrasi = async (e: React.FormEvent) => {
    ...
    setLoading(false);
    if (res.ok) { setMsg({ type: 'ok', text: 'Donatur baru berhasil didaftarkan.' }); setFormBaru({ date: new Date().toISOString().split('T')[0], name: '' }); onRefreshDonors(); }
      ...
      {view === 'tx' ? (
        ...
        <div style={{ overflowX: 'auto', paddingBottom: '120px', marginBottom: '-120px' }}>
          <table className="admin-table">
            ...
            <tbody>
              {fixedDonors.length === 0 && <tr><td colSpan={3} style={{ textAlign: 'center', color: '#9ca3af', padding: '32px' }}>Belum ada donatur tetap terdaftar.</td></tr>}
              {fixedDonors.map(fd => (
                <tr key={fd.id}>
    ...
    <div className="admin-root" style={{ padding: 0 }}>
      ...
      <aside className="admin-sidebar">
        ...
        <div className="sidebar-footer">
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '20px', paddingTop: '16px', borderTop: '1px solid rgba(255,255,255,0.1)', padding: '0 4px' }}>
            <span style={{ fontSize: '10px', opacity: 0.3, color: 'white' }}>Darul Ihsan</span>
            <a href="https://daisabil.com" target="_blank" rel="noopener noreferrer" style={{ fontSize: '10px', opacity: 0.3, textDecoration: 'none', color: 'white' }}>
              ©<b>daisabil 2026</b>
            </a>
          </div>
        </div>
          ...
          <nav className="mobile-drawer">
            ...
            <div style={{ padding: '12px', borderTop: '1px solid var(--border)' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '12px', paddingTop: '12px', borderTop: '1px solid rgba(0,0,0,0.05)' }}>
                <span style={{ fontSize: '10px', opacity: 0.3, color: 'inherit' }}>Darul Ihsan</span>
                <a href="https://daisabil.com" target="_blank" rel="noopener noreferrer" style={{ fontSize: '10px', opacity: 0.3, textDecoration: 'none', color: 'inherit' }}>
                  ©<b>daisabil 2026</b>
                </a>
              </div>
            </div>
